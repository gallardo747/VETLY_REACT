...
import { useTablerosVeter_Turnos } from '../hooks/useTableros';
...
export default function Cobros() {
  ...
  const [usuario, setUsuario] = useState(null);
    ...
    useEffect(() => {
      ...
      // METRICA 1: TURNOS DE VETERINARIA
      const { KPIVeterinaria_TurnosConsolidado, actualizarKPIVeterinaria_Turnos } = useTablerosVeter_Turnos(usuario);
      const [solicitudesLoaded, setsolicitudesLoaded] = useState(false);

      useEffect(() => {
        if (usuario !== null) {
          actualizarKPIVeterinaria_Turnos(usuario);
      ...
      }, [solicitudesLoaded, KPIVeterinaria_TurnosConsolidado, usuario, GLOBALVetly.id, DataAMostrar]);
      ...
      const handleFilter = () => {
        ...
        const filteredItems = KPIVeterinaria_TurnosConsolidado.filter((usuario) => {
      ...
      <Route path='/cobros'>
        ...
        <div className="flex items-center py-10">
            ...
            {GLOBALVetly.type === 'VETERINARIA' && (              
                <FormattedMessage id="HistorialCobros.bienvenida" defaultMessage={welcomeText}/>
            )}
            </h3>
          </div>
          ...
          {GLOBALVetly.type === 'VETERINARIA' && (
            <div className="date-filter">
              <label htmlFor="startDate">
                  <FormattedMessage id="HistorialCobros.desde" defaultMessage="Fecha Desde:"/>
             </label>
              <input
                type="date"
                id="startDate"
              ...
              <label htmlFor="endDate">
                  <FormattedMessage id="HistorialCobros.hasta" defaultMessage="Fecha Hasta:"/>
              </label>
              <input
                type="date"
                id="endDate"
              ...
              <button className="btn btn-primary" onClick={handleFilter} disabled={!startDate}>
                  <FormattedMessage id="HistorialCobros.filtro" defaultMessage="Filtrar"/> 
                </button>
          ...
          {GLOBALVetly.type === 'VETERINARIA' && DataAMostrar && (    
            ...
            <thead className="table-sm">
              <tr>
                ...
                <th scope="col" className="tarifa-column"  onClick={() => handleSort('tarifa')}>
                  <FormattedMessage id="HistorialCobros.tarifa" defaultMessage="TARIFA"/> 
                  <i className={getSortIcon('tarifa')}></i>
                </th>
                <th scope="col" onClick={() => handleSort('apellido')}>
                  <FormattedMessage id="HistorialCobros.profesional" defaultMessage="PROFESIONAL"/> 
              ...
              <tfoot>
                  <tr>
                    ...
                    <td className="tarifa-value">
                      <strong>
                            <FormattedMessage id="HistorialCobros.total" defaultMessage="Total Acumulado: $"/> 
